...
export default function RolePermissionsModal({ isOpen, onClose, onSuccess, readOnly = false }) {
    ...
    const handleSubmit = async (selectedPermissionIds) => {
        ...
        try {
            ...
            const permissionNames = selectedPermissionIds.map(id => {
                let permissionName;
    ...
    if (!selectedRole?.id || !permissions || Object.keys(permissions).length === 0) {
        return null;
    }
