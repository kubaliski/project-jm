...
export default function RolePermissionsForm({
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const errorMessage = error?.response?.data?.message || 'Ocurrió un error al guardar los permisos.';
            setErrors({
            ...
            {errors.general && (
                <div className="bg-red-50 border-l-4 border-red-400 p-4">
                    <div className="flex">
                        ...
                        <div className="ml-3">
                            <p className="text-sm text-red-700">{renderError(errors.general)}</p>
                        </div>
                    </div>
                </div>
            )}
                        ...
                        <div className="bg-gray-50 px-4 py-3">
                            <div className="w-full flex items-center justify-between">
                                ...
                                >
                                    ...
                                    <div className="flex items-center space-x-2">
                                        ...
                                        <span className="px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-600">
                                            {groupPermissions.filter(p => selectedPermissions.has(p.id)).length} / {groupPermissions.length}
                                        </span>
                                    </div>
                                </div>
                                ...
                                {!readOnly && (
                                    <button
                                        type="button"
                                        onClick={(e) => handleSelectAllGroup(groupPermissions, e)}
                                        className="text-sm text-indigo-600 hover:text-indigo-800 font-medium"
                                    >
                                        {groupPermissions.every(p => selectedPermissions.has(p.id))
                                            ? 'Deseleccionar todo'
                                            : 'Seleccionar todo'
                                        }
                                    </button>
